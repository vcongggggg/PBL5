...
class TestMqttParkingLogic(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_02b_rfid_early_swipe_processed_after_lpr_completes(self):
        device_id = "esp32-barrier-01"
        card = self.add_guest_card("EARLYOK")
        pending = models.PendingScan(
            gate_type="entry",
            plate_number="51F88801",
            confidence=0.95,
            device_id=device_id,
            scan_token="early-ok",
        )
        self.db.add(pending)
        self.db.commit()
        main_module._pending_rfid_scans["entry"] = ("EARLYOK", device_id, get_vietnam_now())
        main_module._pending_rfid_scans.pop("entry")

        await main_module.process_mqtt_rfid_validation(self.db, pending, "EARLYOK", "entry", device_id, "in")

        main_module.mqtt_manager.publish_open_gate.assert_called_with(device_id, "in")
        self.assertNotIn("entry", main_module._pending_rfid_scans)
        self.assertEqual(self.db.query(models.PendingScan).count(), 0)
        session = self.db.query(models.ParkingSession).filter(models.ParkingSession.rfid_tag == "EARLYOK").first()
        self.assertIsNotNone(session)
        self.assertEqual(session.plate_number, "51F88801")
        self.db.refresh(card)
        self.assertEqual(card.status, "in_use")
    ...
    async def test_20_duplicate_plate_and_wrong_rfid_exit_edges(self):
        ...
        self.assertIn("đang có phiên", duplicate.message.lower())
    ...
    async def test_29_locked_card_mid_session_requires_manual_exit(self):
        card = self.add_guest_card("LOCKMID", status="in_use")
        self.add_open_session("51F10101", "LOCKMID", card)
        card.is_active = False
        self.db.commit()

        result = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="exit", trigger_type="rfid",
            source_id="test", rfid_tag="LOCKMID", override_plate="51F10101", override_confidence=0.95,
        )
        self.assertEqual(result.action, "ignore")
        self.assertIn("bị khóa", result.message.lower())

    async def test_30_exit_available_rfid_without_entry_session_rejected(self):
        self.add_guest_card("NOENTRY", status="available")
        result = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="exit", trigger_type="rfid",
            source_id="test", rfid_tag="NOENTRY", override_plate="51F20202", override_confidence=0.95,
        )
        self.assertEqual(result.action, "ignore")
        self.assertIn("không tìm thấy", result.message.lower())

    async def test_31_partial_plate_ocr_rejected(self):
        self.add_guest_card("PARTIAL")
        result = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="entry", trigger_type="rfid",
            source_id="test", rfid_tag="PARTIAL", override_plate="51F123", override_confidence=0.95,
        )
        self.assertEqual(result.action, "ignore")

    async def test_32_monthly_and_guest_capacity_are_separate(self):
        guest_cfg = self.db.query(models.SystemConfig).filter(models.SystemConfig.key == "max_guest_slots").first()
        monthly_cfg = self.db.query(models.SystemConfig).filter(models.SystemConfig.key == "max_monthly_slots").first()
        guest_cfg.value = "1"
        monthly_cfg.value = "1"
        self.db.commit()

        guest_card = self.add_guest_card("GSPACE", status="in_use")
        self.add_open_session("51F30301", "GSPACE", guest_card)
        monthly_user, vehicle, sub, monthly_card = self.add_monthly_bundle(uid="MSPACE", plate="51F30302")

        guest_new = self.add_guest_card("GSPACE2")
        guest_blocked = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="entry", trigger_type="rfid",
            source_id="test", rfid_tag="GSPACE2", override_plate="51F30303", override_confidence=0.95,
        )
        self.assertEqual(guest_blocked.action, "ignore")
        self.assertIn("Bai xe da day", guest_blocked.message)

        monthly_allowed = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="entry", trigger_type="rfid",
            source_id="test", rfid_tag="MSPACE", override_plate="51F30302", override_confidence=0.95,
        )
        self.assertEqual(monthly_allowed.action, "open")

        stats = main_module.get_dashboard_stats(db=self.db)
        self.assertEqual(stats.guest_in_bay, 1)
        self.assertEqual(stats.monthly_in_bay, 1)
        self.assertEqual(stats.available_guest_slots, 0)
        self.assertEqual(stats.available_monthly_slots, 0)

    async def test_33_fire_critical_opens_both_gates_once_per_cooldown(self):
        main_module._last_fire_gate_open_at = 0.0
        main_module.mqtt_manager.is_connected = True
        await handle_mqtt_event("fire-critical", "fire_alert", {"sensor_value": 0, "message": "Lua lon"})
        main_module.mqtt_manager.publish_open_gate.assert_any_call("esp32-barrier-01", "in")
        main_module.mqtt_manager.publish_open_gate.assert_any_call("esp32-barrier-01", "out")
        self.assertEqual(main_module.mqtt_manager.publish_open_gate.call_count, 2)

        await handle_mqtt_event("fire-critical", "fire_alert", {"sensor_value": 0, "message": "Lua lon tiep"})
        self.assertEqual(main_module.mqtt_manager.publish_open_gate.call_count, 2)

    async def test_34_force_checkout_missing_plate_returns_404(self):
        with self.assertRaises(main_module.HTTPException) as ctx:
            await main_module.force_checkout(
                plate_number="51F40404", reason="lost_card", open_gate=False, db=self.db, api_key="pbl5_secure_key_12345"
            )
        self.assertEqual(ctx.exception.status_code, 404)

    async def test_35_mqtt_publish_open_gate_correct_direction_after_rfid_validation(self):
        card = self.add_guest_card("DIRIN")
        pending = models.PendingScan(gate_type="entry", plate_number="51F50505", confidence=0.95, device_id="esp32", scan_token="dir-in")
        self.db.add(pending)
        self.db.commit()
        await handle_mqtt_event("esp32", "rfid_scan", {"uid": "DIRIN", "direction": "in"})
        main_module.mqtt_manager.publish_open_gate.assert_called_with("esp32", "in")

        main_module.mqtt_manager.publish_open_gate.reset_mock()
        card.status = "in_use"
        self.add_open_session("51F60606", "DIRIN", card)
        pending = models.PendingScan(gate_type="exit", plate_number="51F60606", confidence=0.95, device_id="esp32", scan_token="dir-out")
        self.db.add(pending)
        self.db.commit()
        await handle_mqtt_event("esp32", "rfid_scan", {"uid": "DIRIN", "direction": "out"})
        main_module.mqtt_manager.publish_open_gate.assert_called_with("esp32", "out")

    async def test_36_mqtt_disconnected_does_not_publish_gate_command(self):
        main_module.mqtt_manager.is_connected = False
        main_module.esp32_ip = None
        with self.assertRaises(main_module.HTTPException) as ctx:
            await force_open_gate(gate_type="entry", reason="test", operator="tester", db=self.db, api_key="pbl5_secure_key_12345")
    ...
    def test_37_dashboard_counts_open_sessions_and_today_totals(self):
        ...
        self.db.commit()
        stats = main_module.get_dashboard_stats(db=self.db)
        self.assertEqual(stats.total_in_bay, 1)
    ...
    async def test_40_monthly_exit_integration_fee_zero_and_type_recorded(self):
        _, _, _, card = self.add_monthly_bundle(uid="MEXITOK", plate="51F90901")
        entry = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="entry", trigger_type="rfid",
            source_id="test", rfid_tag="MEXITOK", override_plate="51F90901", override_confidence=0.95,
        )
        self.assertEqual(entry.action, "open")
        session = self.db.query(models.ParkingSession).filter(models.ParkingSession.id == entry.session_id).first()
        self.assertEqual(session.rfid_card_type, "monthly")

        exit_ = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="exit", trigger_type="rfid",
            source_id="test", rfid_tag="MEXITOK", override_plate="51F90901", override_confidence=0.95,
        )
        self.assertEqual(exit_.action, "open")
        self.assertEqual(exit_.fee, 0)
        self.db.refresh(session)
        self.db.refresh(card)
        self.assertEqual(session.match_status, "matched")
        self.assertEqual(card.status, "available")

    async def test_41_monthly_expired_after_entry_can_exit_as_guest_fee(self):
        _, _, sub, card = self.add_monthly_bundle(uid="MEXITAFT", plate="51F90902")
        entry = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="entry", trigger_type="rfid",
            source_id="test", rfid_tag="MEXITAFT", override_plate="51F90902", override_confidence=0.95,
        )
        self.assertEqual(entry.action, "open")
        session = self.db.query(models.ParkingSession).filter(models.ParkingSession.id == entry.session_id).first()
        session.time_in = get_vietnam_now() - timedelta(minutes=61)
        sub.end_date = main_module.get_vietnam_date() - timedelta(days=1)
        self.db.commit()
        ...
        self.assertEqual(exit_.action, "open")
        self.assertGreater(exit_.fee or 0, 0)
        self.db.refresh(card)
        self.assertEqual(card.status, "available")
    ...
    async def test_43_exit_fuzzy_boundary_camera_vs_rfid(self):
        ...
        self.add_open_session("51F12345", "FUZZS", card_sensor)
        sensor_exit = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="exit", trigger_type="sensor",
            source_id="test", rfid_tag=None, override_plate="51F12367", override_confidence=0.95,
        )
        self.assertEqual(sensor_exit.action, "ignore")

        card_rfid = self.add_guest_card("FUZZR", status="in_use")
        self.add_open_session("51F22345", "FUZZR", card_rfid)
        rfid_exit = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="exit", trigger_type="rfid",
            source_id="test", rfid_tag="FUZZR", override_plate="51F22367", override_confidence=0.95,
        )
        self.assertEqual(rfid_exit.action, "open")
        self.assertEqual(rfid_exit.matched, True)

    async def test_44_force_checkout_open_gate_true_publishes_mqtt_and_fee(self):
        card = self.add_guest_card("FORCEOPEN", status="in_use")
        session = self.add_open_session("51F91919", "FORCEOPEN", card)
        session.time_in = get_vietnam_now() - timedelta(minutes=61)
        self.db.commit()
    ...
    async def test_46_config_zero_capacity_blocks_entry_and_threshold_zero_still_rejects_unknown(self):
        for key in ["max_parking_slots", "max_guest_slots"]:
            ...
            cfg.value = "0"
        threshold = self.db.query(models.SystemConfig).filter(models.SystemConfig.key == "plate_confidence_threshold").first()
        threshold.value = "0"
        self.db.commit()

        card = self.add_guest_card("ZEROCAP")
        blocked = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="entry", trigger_type="rfid",
            source_id="test", rfid_tag="ZEROCAP", override_plate="51F92929", override_confidence=0.95,
        )
        self.assertEqual(blocked.action, "ignore")

        unknown_card = self.add_guest_card("UNKTHR")
        unknown = await process_gate_scan(
            db=self.db, image_bytes=None, filename=None, gate_type="entry", trigger_type="rfid",
            source_id="test", rfid_tag="UNKTHR", override_plate="UNKNOWN", override_confidence=0.0,
        )
    ...
    async def test_47_mqtt_malformed_payloads_and_cooldown_unblocks_after_window(self):
        ...
        try:
            with patch("app.main.asyncio.create_task", side_effect=capture_task), patch("app.main.time_module.time", side_effect=[100.0, 100.2, 101.2]):
                await handle_mqtt_event("esp32", "car_detected", {})
                await handle_mqtt_event("esp32", "car_detected", {})
                await handle_mqtt_event("esp32", "car_detected", {})
        ...
        await handle_mqtt_event("fire-empty", "fire_alert", {"sensor_value": 0})
        alert = self.db.query(models.FireAlert).filter(models.FireAlert.sensor_id == "fire-empty").first()
        self.assertIsNotNone(alert)
    ...
    def test_48_dashboard_yesterday_entry_today_exit_and_type_counts(self):
        ...
        self.db.add(models.ParkingSession(
            ...
            time_in=now - timedelta(days=1, hours=1),
            time_out=now - timedelta(minutes=5),
            fee=5000,
            match_status="matched",
            rfid_card_type="guest",
        ))
        guest_card = self.add_guest_card("COUNTG", status="in_use")
        guest_open = self.add_open_session("51F93932", "COUNTG", guest_card)
        guest_open.time_in = now - timedelta(days=1, hours=2)
        _, _, _, monthly_card = self.add_monthly_bundle(uid="COUNTM", plate="51F93933")
        monthly_card.status = "in_use"
        monthly_open = self.add_open_session("51F93933", "COUNTM", monthly_card)
        monthly_open.time_in = now - timedelta(days=1, hours=1)
        monthly_open.rfid_card_type = "monthly"
        self.db.commit()

        stats = main_module.get_dashboard_stats(db=self.db)
        self.assertEqual(stats.today_total_in, 0)
        self.assertEqual(stats.today_total_out, 1)
        self.assertEqual(stats.guest_in_bay, 1)
        self.assertEqual(stats.monthly_in_bay, 1)
